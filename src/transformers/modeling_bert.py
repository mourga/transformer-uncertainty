...
from transformers.file_utils import ModelOutput, add_start_docstrings, add_start_docstrings_to_callable, \
    add_code_sample_docstrings, replace_return_docstrings
from transformers.modeling_outputs import BaseModelOutput, BaseModelOutputWithPooling, CausalLMOutput, MaskedLMOutput, \
    NextSentencePredictorOutput, SequenceClassifierOutput, MultipleChoiceModelOutput, TokenClassifierOutput, \
    QuestionAnsweringModelOutput
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        ##########################################################################################
        self.adapter = None
        self.bayes_adapter = None
        if hasattr(config, "use_adapter"):
            self.adapter = Adapter(config) if config.use_adapter else None
        if hasattr(config, "use_bayes_adapter"):
            self.bayes_adapter = BayesAdapter(config) if config.use_bayes_adapter else None
        ##########################################################################################
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
