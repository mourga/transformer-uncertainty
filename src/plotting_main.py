...
def ac_ece_table(datasets, models, indicators, seeds=[2, 19, 729, 982, 75],
    ...
    if ood:
        ...
        ## prepare bayes df for merging
        map_bay_meth_for_merge = {
            "vanilla": "UA",
            "mc5" : "UA + MC-5"
        }

        df_bay_model["method"] = df_bay_model.method.apply(lambda x: map_bay_meth_for_merge[x])
        df_bay_model["indicator"] = "Base"

        ## concatenate both
        df_base_bay = pd.concat([df_bay_model, df_model])[["model_type", "method","dataset", "test_acc", "test_ece"]]

        df_base_bay = df_base_bay.groupby(["model_type", "method", "dataset"]).mean()

        df_base_bay = pd.DataFrame(df_base_bay)
        df_base_bay.reset_index(inplace=True)

        new_results = {}

        ### oriantate them in the way we want
        for task_name in df_base_bay.dataset.unique():

            new_results[task_name] = {}
        ...
        new_results.to_csv(os.path.join(path, 'ac_ece_ood_detailed.csv'), index = True)

        new_results.to_latex(os.path.join(path, 'ac_ece_ood_detailed.tex'), index = True)

        new_results.mean(1).to_csv(os.path.join(path, 'ac_ece_ood_summary.csv'), index = True)
        ...
        ## concatenate both
        df_base_bay = pd.concat([df_bay_model, df_model])

        df_base_bay = df_base_bay.groupby(["model_type", "dataset", "method"]).mean()
        ...
        for task_name in df_base_bay.dataset.unique():
            ...
            acc_results[task_name] = {}
            ece_results[task_name] = {}


            for model_ in ["bert", "distilbert"]:
                for method in ["vanilla", "temp_scale", "mc5", "mc20", "UA", "UA + MC-5"]:
                
                    
                    try:

                        acc_results[task_name][model_ + "-" + method] = (df_base_bay[(df_base_bay.dataset == task_name) & (df_base_bay.model_type == model_) & (df_base_bay.method == method)]["test_acc"]*100).round(1).values[0]
                        ece_results[task_name][model_ + "-" + method] = (df_base_bay[(df_base_bay.dataset == task_name) & (df_base_bay.model_type == model_) & (df_base_bay.method == method)]["test_ece"]*100).round(2).values[0]

                    except:

                        acc_results[task_name][model_ + "-" + method] = np.nan
                        ece_results[task_name][model_ + "-" + method] = np.nan
        ...
        acc_results["metric"] = "accuracy"
        ece_results = pd.DataFrame(ece_results)
        ece_results["metric"] = "ece"

        new_results = pd.concat([acc_results, ece_results])
        new_results["avg"] = new_results.mean(1).round(2)
